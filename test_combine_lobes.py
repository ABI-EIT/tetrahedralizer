...
def main():
    ...
    shared_points = []
    for i_a, point_a in enumerate(meshes[0].points):
        for i_b, point_b in enumerate(meshes[1].points):
    ...
    for i, mesh in enumerate(meshes):
        ...
        mesh_shared_points_set = set(np.asarray(shared_points)[:, i])
        for j, face in enumerate(faces):
            # Check if all of face are in the current meshes shared points
            if set(face).issubset(mesh_shared_points_set):
                mesh_shared_faces.append(j)
        shared_faces.append(mesh_shared_faces)
